...
parser.add_argument('--entree', type=str)
...
if args.entree is None:
    ...
    webcam=cv2.VideoCapture(args.entree)
    webcam.set(cv2.CAP_PROP_FRAME_WIDTH,WF)
    webcam.set(cv2.CAP_PROP_FRAME_HEIGHT, HF)
    args.mode='video'
'''
...
############## AFFICHAGE ####################    
    ...
    CouleurTexte = [255,255,255]
    EpaisseurTexte = 2
    cv2.putText(bordered_frame, Chrono, Position, Police, TaillePolice, CouleurTexte, EpaisseurTexte)
    ...
    if args.entree is None:
        ...
        cv2.imshow('Video', bordered_frame) 
############## AFFICHAGE ####################         
...
# Sauvegarde en pdf
plt.savefig('Test_'+ args.mode +'.pdf')
