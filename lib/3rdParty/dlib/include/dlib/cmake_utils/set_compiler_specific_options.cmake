...
# Setup options for SIMD instruction sets, conditionally based on architecture
...
elseif (("${CMAKE_SYSTEM_PROCESSOR}" MATCHES "^arm")) # If on ARM architecture
...
if (CMAKE_COMPILER_IS_GNUCXX)
   # Add warning for missing return type in GCC
   list(APPEND active_compile_opts "-Wreturn-type")
endif()

if ("Clang" MATCHES ${CMAKE_CXX_COMPILER_ID})
   # Increase clang's default template recursion depth so the dnn examples don't error out.
   list(APPEND active_compile_opts "-ftemplate-depth=500")
endif()

if (MSVC)
   # Handle large object files for Visual Studio
   list(APPEND active_compile_opts "/bigobj")

   # Build dlib with all cores, but don't propagate this setting to client programs
