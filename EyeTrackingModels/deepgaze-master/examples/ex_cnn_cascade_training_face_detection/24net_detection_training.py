...
def main():  
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.get_variable("conv1_24d_w", [5, 5, num_channels, 64], initializer=tf_initializer)
            conv1_biases = tf.Variable(tf.zeros([64]), name="conv1_24d_b")
            # Dense layer
            # [ 5*5 * previous_layer_out , num_hidden] wd1
            # after pooling the 24x24 image is reduced to size 12x12
            dense1_weights = tf.get_variable("dense1_24d_w", [12 * 12 * 64, 128], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random_normal(shape=[128]), name="dense1_24d_b")
            # Output layer
            layer_out_weights = tf.get_variable("out_24d_w", [128, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random_normal(shape=[num_labels]), name="out_24d_b")
            ...
            with tf.Session(graph=graph) as session:
                ...
                log_path = "./logs/log_24net_detection_" + str(now.hour) + str(now.minute) + str(now.second)
                ...
                checkpoint_path = "./checkpoints/24net_detection_" + str(now.hour) + str(now.minute) + str(now.second) 
                if not os.path.exists(checkpoint_path):
                    os.makedirs(checkpoint_path) 
                saver.save(session, checkpoint_path + "/cnn_24net_detection" , global_step=step)  # save the session
