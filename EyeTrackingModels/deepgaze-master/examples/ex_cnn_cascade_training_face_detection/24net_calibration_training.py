...
def main():  
        ...
        pickle_file_positive = "./positive_dataset_24net_65375.pickle"
        pickle_file_negative = "./negative_dataset_24net_198081.pickle"
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.get_variable("conv1_24c_w", [5, 5, num_channels, 32], initializer=tf_initializer)
            conv1_biases = tf.Variable(tf.zeros([32]), name="conv1_24c_b")
            # Dense layer
            # [ 5*5 * previous_layer_out , num_hidden] wd1
            # after pooling the 24x24 image is reduced to size 12x12
            dense1_weights = tf.get_variable("dense1_24c_w", [12 * 12 * 32, 64], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random_normal(shape=[64]), name="dense1_24c_b")
            # Output layer
            layer_out_weights = tf.get_variable("out_24c_w", [64, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random_normal(shape=[num_labels]), name="out_24c_b")
            ...
            with tf.Session(graph=graph) as session:
                ...
                log_path = "./logs/log_24net_calibration_" + str(now.hour) + str(now.minute) + str(now.second)
                ...
                checkpoint_path = "./checkpoints/24net_calibration_" + str(now.hour) + str(now.minute) + str(now.second) 
                if not os.path.exists(checkpoint_path):
                    os.makedirs(checkpoint_path) 
                saver.save(session, checkpoint_path + "/cnn_24net_calibration" , global_step=step)  # save the session
