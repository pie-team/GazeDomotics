...
def main():            
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.get_variable("conv1_12c_w", [3, 3, num_channels, 16], initializer=tf_initializer)
            conv1_biases = tf.Variable(tf.zeros([16]), name="conv1_12c_b")
            # Dense layer
            # [ 5*5 * previous_layer_out , num_hidden] wd1
            # after pooling the 12x12 image is reduced to size 6x6
            dense1_weights = tf.get_variable("dense1_12c_w", [6 * 6 * 16, 128], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random_normal(shape=[128]), name="dense1_12c_b")
            # Output layer
            layer_out_weights = tf.get_variable("out_12c_w", [128, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random_normal(shape=[num_labels]), name="out_12c_b")
            ...
            with tf.Session(graph=graph) as session:
                ...
                log_path = "./logs/log_12net_calibration_" + str(now.hour) + str(now.minute) + str(now.second)
                ...
                checkpoint_path = "./checkpoints/12net_calibration_" + str(now.hour) + str(now.minute) + str(now.second) 
                if not os.path.exists(checkpoint_path):
                    os.makedirs(checkpoint_path) 
                saver.save(session, checkpoint_path + "/cnn_12net_calibration" , global_step=step)  # save the session
