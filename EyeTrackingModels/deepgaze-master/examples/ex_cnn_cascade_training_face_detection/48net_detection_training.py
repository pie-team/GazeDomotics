...
def main():            
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.get_variable("conv1_48d_w", [5, 5, num_channels, 64], initializer=tf_initializer)
            conv1_biases = tf.Variable(tf.zeros([64]), name="conv1_48d_b")
            # Conv layer
            # [patch_size, patch_size, num_channels, depth]
            conv2_weights = tf.get_variable("conv2_48d_w", [5, 5, 64, 64], initializer=tf_initializer)
            conv2_biases = tf.Variable(tf.zeros([64]), name="conv2_48d_b")
            # Dense layer
            # [ 5*5 * previous_layer_out , num_hidden] wd1
            # after 2 poolings the 48x48 image is reduced to size 12x12
            dense1_weights = tf.get_variable("dense1_48d_w", [12 * 12 * 64, 256], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random_normal(shape=[256]), name="dense1_48d_b")
            # Output layer
            layer_out_weights = tf.get_variable("out_48d_w", [256, num_labels], initializer=tf_initializer)
